...
from processNano.corrections.fake_factors import fake_factors
...
#                & muons.pass_flags
            ...
            #muons = muons[muons.selection & (nmuons == 2) & (abs(sum_charge) >=2 ) & (hlt > 0)  ]
            #muons = muons[muons.selection & (nmuons >= 2) & (abs(sum_charge) < nmuons) & (hlt > 0) & (muons.overlap) ]

            #print(" length ", muons.charge)
            ...
            if ((self.calc_fake == True)):
                ...
                mu1["FR1"] =  ((mu1.highPtId == 0) & (mu2.highPtId > 0))
                mu1["FR2"] =  ((mu1.highPtId > 0)  & (mu2.highPtId == 0))
                mu1["FR12"] = ((mu1.highPtId == 0) & (mu2.highPtId == 0))
                mu1["no_FR"] = ((mu1.highPtId >0) & (mu2.highPtId >0))
                print(mu1.highPtId)
                print(mu2.highPtId)
                
                for index, row in mu1.iterrows():
                    ...
                    if row['FR1'] == True:
                        ...
                        FR_val1 = fake_factors(mu1.pt[index], mu1.eta[index])
                    ...
                    if(row['FR2'] == True):
                        ...
                        FR_val2 = fake_factors(mu2.pt[index], mu2.eta[index])
