...
lineColors = {"ConLL":ROOT.kRed,"ConLR":ROOT.kRed,"ConRL":ROOT.kRed,"ConRR":ROOT.kRed,"negLL":ROOT.kBlue,"negLR":ROOT.kBlue,"negRL":ROOT.kBlue,"negRR":ROOT.kBlue}
lineStyles = {"ConLL":1,"ConLR":2,"ConRL":2,"ConRR":4,"DesLL":1,"DesLR":2,"DesRL":2,"DesRR":4}
labels = {"negLL":"destructive left-left","ConLR":"constructive left-right","ConRL":"constructive right-left","ConRR":"constructive right-right","DesLL":"destructive left-left","negLR":"destructive left-right","negRL":"destructive right-left","negRR":"destructive right-right"}



def printPlots(canvas,name):
        canvas.Print('plots/'+name+"_Logy.png","png")
        canvas.Print('plots/'+name+"_Logy.pdf","pdf")
        canvas.SaveSource('plots/'+name+"_Logy.C","cxx")
        canvas.Print('plots/'+name+"_Logy.root","root")
        canvas.Print('plots/'+name+"_Logy.eps","eps")
...
def makeLimitPlot(output,obs,exp,chan,interference,printStats=False,obs2="",ratioLabel=""):
        ...
        observedy=[]
        obsLimits={}
        xSecs = getXSecCurve("CI_%s"%interference,1.0)
        ...
        cCL=TCanvas("cCL", "cCL",0,0,800,800)
        W = 800
        H  = 800
        T = 0.08*H
        B = 0.12*H
        L = 0.12*W
        R = 0.04*W
        cCL.SetLeftMargin( L/W )
        cCL.SetRightMargin( R/W )
        cCL.SetTopMargin( T/H )
        cCL.SetBottomMargin( B/H )
        ...
        xSecCurves.append(getXSecCurve("CI_%s"%interference,1.0)) 

        #Draw the graphs:
        plotPad.SetLogy()
        DummyGraph=TH1F("DummyGraph","",100,4,18)
        #DummyGraph=TH1F("DummyGraph","",100,4,14)
        ...
        gStyle.SetOptStat(0)
        DummyGraph.GetXaxis().SetRangeUser(4,18)
        #DummyGraph.GetXaxis().SetRangeUser(4,14)

        DummyGraph.SetMinimum(0.01)
        DummyGraph.SetMaximum(1000)
        ...
        DummyGraph.GetYaxis().SetTitleOffset(1.1)
        DummyGraph.Draw()
        if (FULL):
                GraphErr2Sig.Draw("F")
                GraphErr1Sig.Draw("F")
        ...
        leg=TLegend(0.15,0.573051,0.375,0.808644,labels[interference],"brNDC")  
...
#       leg=TLegend(0.55,0.55,0.87,0.87,"","brNDC")   
        ...
        if "Moriond" in output:
                ...
                if (chan=="mumu"): 
                        plLumi=TPaveLabel(.52,.901,.9,.99,"138 fb^{-1} (13 TeV, #mu#mu)","NBNDC")
                elif (chan=="elel"):
                        plLumi=TPaveLabel(.52,.901,.9,.99,"138 fb^{-1} (13 TeV, ee)","NBNDC")
                elif (chan=="elmu"):
                        plLumi=TPaveLabel(.4,.901,.9,.99,"12.4 fb^{-1} (13 TeV, ee) + 13.0 fb^{-1} (13 TeV, #mu#mu)","NBNDC")

        plLumi.SetTextSize(0.43)
