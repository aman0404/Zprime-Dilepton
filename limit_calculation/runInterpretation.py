    ...
    #            interferences = config.interferencesBBLL
        for Lambda in Lambdas:
            for interference in interferences:
                ...
                import random
                print ("running toys for Lambda %d and model %s"%(Lambda,interference))
                ...
                #if args.workDir == "":
                subprocess.call(["mv","higgsCombine%s_%d.GenerateOnly.mH%d.123456.root"%(args.config,number,Lambda),outDir+"/higgsCombine%s_%s.GenerateOnly.mH%d.123456.root"%(args.config,interference,Lambda)])
                ...
                print ("ready toys for Lambda %d and model %s"%(Lambda,interference))
                #else:	
        ...
        for Lambda in Lambdas:
            for interference in interferences:
                ...
                if args.expected and not args.asymptotic: 
                    ...
                    subCommand.append("-s")
                    subCommand.append("0")
                    if args.is_int: ##Aman edits here 
                        subCommand.append("--toysFile="+cardDir+"_expGen"+"/higgsCombine%s_%s.GenerateOnly.mH%d.123456.root"%(args.config,interference,Lambda))
        ...
        for Lambda in Lambdas:
            ...
            for interference in interferences:
                if len(config.channels) == 1:
                    ...
                    cardName = args.config + "_combined" + "_%d_%s"%(Lambda,interference) + "_shape.txt"
                if binned:
                    cardName = cardName.split(".")[0] + "_binned.txt"
                ...
                if config.submitTo == "Purdue":
                    if args.is_int:  ##Aman edits here
                        ...
                        if args.expected:
                            ...
                            for i in range(0,numJobs):
                                subCommand = "sbatch --mem=4G -N1 -n1 --time=48:00:00 -A cms --reservation=CMSLOCAL %s/submission/CILimitsInt_Exp_PURDUE.job %s %s %s %s %d %d %d %d %d %s %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,i,50,Lambda,getRange(Lambda,True,args.usePhysicsModel, BSLL=args.BSLL),timestamp,interference)
                            ...
                            #for i in range(0,config.numToys):
                            subCommand = "sbatch --mem=4G -N1 -n1 --time=48:00:00  -A cms --reservation=CMSLOCAL %s/submission/CILimitsInt_PURDUE.job %s %s %s %s %d %d %d %d %d %s %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,config.numToys,0,Lambda,getRange(Lambda,True,args.usePhysicsModel, BSLL=args.BSLL),timestamp,interference)
                            print (subCommand)
                            subprocess.call(subCommand,shell=True)	
                            import time
                            time.sleep(0.1)	
                        ...
                        if args.expected:
                            ...
                            for i in range(0,numJobs):
                                ...
                                if args.BSLL:
                                    ...
                                    subCommand = "sbatch --mem=4G -N1 -n1 -A cms  --reservation=CMSLOCAL --time=48:00:00  %s/submission/CILimits_PURDUE.job %s %s %s %s %d %d %d %d %d %s %d %d %s"%(srcDir,args.config,name+"_"+interference,srcDir,cardName,config.numInt,i,50,Lambda,getRange(Lambda,True,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.singlebin,args.mass,Libs)
                                print ("subCommand: {0}".format(subCommand))
                                subprocess.call(subCommand,shell=True)			
                        else:
                           if args.BSLL:
                                ...
                                subCommand = "sbatch -N1 -n1 --time=48:00:00 -A cms --reservation=CMSLOCAL %s/submission/CILimits_PURDUE.job %s %s %s %s %d %d %d %d %d %s %d %d %s"%(srcDir,args.config,name+"_"+interference,srcDir,cardName,config.numInt,config.numToys,0,Lambda,getRange(Lambda,True,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.singlebin,args.mass,Libs)
        ...
        for massRange in masses:
            ...
            while mass <= massRange[2]:
                ...
                if config.submitTo == "Purdue":
                    if args.frequentist:
                        if args.expected:
                            subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimeLimitsFreq_PURDUE.job %s %s %s %s %d %d %d %d %d %s %d %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,config.numToys,config.exptToys,mass,getRange(mass,False,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.spin2,Libs)
                            subprocess.call(subCommand,shell=True)			
                        else:
                            for i in range(0,config.numToys):
                                subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimeLimitsFreq_PURDUE.job %s %s %s %s %d %d %d %d %d %s %d %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,i,0,mass,getRange(mass,False,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.spin2,Libs)
                                subprocess.call(subCommand,shell=True)			
                    elif args.asymptotic:
                        
                        subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimeLimitsAsymp_PURDUE.job %s %s %s %s %d %d %d %d %d %s %d %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,0,0,mass,getRange(mass,False,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.spin2,Libs)
                        subprocess.call(subCommand,shell=True)			
                        ...
                        if args.expected:
                            ...
                            for i in range(1,numJobs+1):
                                if args.BSLL:
                                    subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimeLimits_PURDUE_BSLL.job %s %s %s %s %d %d %d %d %d %s %d %d %d %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,config.numToys,config.exptToys/500,mass,getRange(mass,False,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.spin2,args.inject,i,Libs)
                                else: #bbll
                                    subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimeLimits_PURDUE.job %s %s %s %s %d %d %d %d %d %s %d %d %d %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,config.numToys,config.exptToys/500,mass,getRange(mass,False,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.spin2,args.inject,i,Libs)
                                subprocess.call(subCommand,shell=True)			
                        else:
                            #for i in range(0,config.numToys):
                            if args.BSLL:
                                subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimeLimits_PURDUE_BSLL.job %s %s %s %s %d %d %d %d %d %s %d %d %d %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,config.numToys,0,mass,getRange(mass,False,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.spin2,args.inject,0,Libs)
                            else:
                                subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimeLimits_PURDUE.job %s %s %s %s %d %d %d %d %d %s %d %d %d %s"%(srcDir,args.config,name,srcDir,cardName,config.numInt,config.numToys,0,mass,getRange(mass,False,args.usePhysicsModel, BSLL=args.BSLL),timestamp,args.spin2,args.inject,0,Libs)
                            subprocess.call(subCommand,shell=True)	
                            import time
                            time.sleep(0.1)		
    ...
    for massRange in masses:
        ...
        if config.submitTo == "Purdue":
            if args.hybrid:
                mass = massRange[1]
                while mass <= massRange[2]:
                    subCommand = "sbatch  --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimePValuesHybrid_PURDUE.job %s %s %s %s %d %s %s"%(srcDir,args.config,name,srcDir,cardName,mass,timestamp,Libs)
                    mass += massRange[0]
                    subprocess.call(subCommand,shell=True)			
            elif args.frequentist:	
                mass = massRange[1]
                while mass <= massRange[2]:
                    subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimePValuesFreq_PURDUE.job %s %s %s %s %d %s %s"%(srcDir,args.config,name,srcDir,cardName,mass,timestamp,Libs)
                    mass += massRange[0]
                    subprocess.call(subCommand,shell=True)			
            elif args.plc:	
                subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimePValuesPLC_PURDUE.job %s %s %s %s %d %d %d %s %s"%(srcDir,args.config,name,srcDir,cardName,massRange[1],massRange[2],massRange[0],timestamp,Libs)
                subprocess.call(subCommand,shell=True)			
            else:
                if args.LEE:
                    subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimePValuesLEE_PURDUE.job %s %s %s %s %d %d %d %s %s"%(srcDir,args.config,name,srcDir,cardName,massRange[1],massRange[2],massRange[0],timestamp,Libs)
                else:	
                    subCommand = "sbatch --mem=4G -N1 -n1 -A cms --time=48:00:00  %s/submission/zPrimePValues_PURDUE.job %s %s %s %s %d %d %d %s %s"%(srcDir,args.config,name,srcDir,cardName,massRange[1],massRange[2],massRange[0],timestamp,Libs)
                subprocess.call(subCommand,shell=True)			
    os.chdir(srcDir)	


def submitLimitsToCrab(args,config,cardDir, tag):
    
    cardDir_Gen = cardDir+"_expGen"
    if (args.is_int and args.expected):
        createInputsGen(args,config,cardDir+"_expGen")
    ...
    if args.redo:
        ...
        if args.BBLL:
            print("args BBLL!")
            createInputsBBLL(args,config,cardDir)
        elif args.BSLL:
            print("args BSLL!")
            createInputsBSLL(args, config, cardDir)
##Aman
        if args.BSLL:
            Lambdas = config.lambdasBSLL
            interferences = config.interferencesBSLL
    ...
    for massRange in masses:
        ...
        while mass < massRange[2]:
            if len(config.channels) ==1:
                command = ["text2workspace.py","%s/%s_%d_%s.txt"%(cardDir,config.channels[0],mass, interferences[0])]
                workspaces.append("%s/%s_%d_%s.root"%(cardDir,config.channels[0],mass, interferences[0]))
                ...
                command = ["text2workspace.py","%s/%s_combined_%d_%s_shape.txt"%(cardDir,args.config, mass, interferences[0])]
                #command = ["text2workspace.py","%s/%s_combined_%d_%s.txt"%(cardDir,args.config, mass, interferences[0])]
                #command = ["text2workspace.py","%s/%s_combined_%d_%s.txt"%(cardDir,args.config,mass, interferences[0]), "-P", "HiggsAnalysis.CombinedLimit.CIInterference:CIInterference"]
                workspaces.append("%s/%s_combined_%d_%s_shape.root"%(cardDir,args.config,mass,interferences[0]))
                #workspaces.append("%s/%s_combined_%d_%s.root"%(cardDir,args.config,mass,interferences[0]))
    ...
    if args.expected:
         for massRange in masses:
            ...
            while mass < massRange[2]:
                ...
                if len(config.channels) > 1:	
                    workspace = "%s/%s_combined_%d_%s_shape.root"%(cardDir,args.config,mass,interferences[0])
                    #workspace = "%s/%s_combined_%d_%s.root"%(cardDir,args.config,mass,interferences[0])
                    #workspace_Gen = "%s/higgsCombine%s_%s.GenerateOnly.mH%s.123456.root"%(cardDir_Gen,args.config,config.interferencess[0],str(mass))
                else:	
                    workspace = "%s/%s_%d_%s.root"%(cardDir,config.channels[0],mass,interferences[0])
                    #workspace_Gen = "%s/higgsCombine%s_%s.GenerateOnly.mH%s.123456.root"%(cardDir_Gen,args.config,config.interferencess[0],str(mass))

                #print("checking Gen workspace ", workspace_Gen, len(workspace_Gen)) 
                print("runInterpretation script flag 2")
                tarCommand = ['tar', '-cvf', 'gridPack.tar',"cfgs/","runInterpretation.py"] + [workspace] 
                #tarCommand = ['tar', '-cvf', 'gridPack.tar',"cfgs/","runInterpretation.py"] + [workspace] + [workspace_Gen]
                ...
                #scriptArgs=["python3",script,"--mass",str(mass),"--nIter",nIter,"--nrJobs",nJobs,"--nToys",nToys,"--gridPack","gridPack.tar","--outputTag",args.tag.split("_")[-1],"--crabConfig","crab_base.py","--config",args.config,"--expected","1","--inject",injectArgument]

                scriptArgs=["python3",script,"--mass",str(mass),"--nIter",nIter,"--nrJobs",nJobs,"--nToys",nToys,"--gridPack","gridPack.tar","--outputTag",args.tag,"--crabConfig","crab_base.py","--config",args.config,"--expected","1","--inject",injectArgument]
        ...
        tarCommand = ['tar', '-cvf', 'gridPack.tar',"cfgs/","runInterpretation.py"] + workspaces 
        #tarCommand = ['tar', '-cvf', 'gridPack.tar',"cfgs/","runInterpretation.py"] + workspaces + workspace_Gen
        ...
        os.chdir("submission/")
        print("here is aman", mass)
#        scriptArgs=["python3",script,"--mass",str(mass),"--nIter",nIter,"--nrJobs",nJobs,"--nToys",nToys,"--gridPack","gridPack.tar","--outputTag",args.tag.split("_")[-1],"--crabConfig","crab_base.py","--config",args.config,"--expected","0","--inject",injectArgument]
        scriptArgs=["python3",script,"--mass",str(mass),"--nIter",nIter,"--nrJobs",nJobs,"--nToys",nToys,"--gridPack","gridPack.tar","--outputTag",args.tag,"--crabConfig","crab_base.py","--config",args.config,"--expected","0","--inject",injectArgument]
        result = subprocess.Popen(scriptArgs,stdout=subprocess.PIPE).communicate()[0].splitlines()
        for line in result:
            print (line)
...
def createInputsBBLL(args,config,cardDir):
    for channel in config.channels:
        print ("writing datacards and workspaces for channel %s ...."%channel)
        if args.singlebin:
            mass = args.mass
            ...
            if args.is_int: ##Aman edits here
                call = ["python3","writeDataCardsCIInt.py","-c","%s"%channel,"-o","%s"%args.config,"-t","%s"%args.tag]
            else:
                print("BBLL contact interaction")
                call = ["python3","writeDataCardsCI.py","-c","%s"%channel,"-o","%s"%args.config,"-t","%s"%args.tag]
        if args.expected:
            call.append("--expected")
        if args.signif:
            call.append("-s")
            ...
            for interference in config.interferencesBBLL:
                ...
                with open('%s'%outName, "w") as outfile:
                    ...
                    if args.usePhysicsModel or args.is_int:
                         ...
                         subprocess.call(command)
                         print("here is aman done ")
                    else:
                         subprocess.call(command, stdout=outfile,cwd=cardDir)
                         outfile_shape = outName.split(".")[0]+"_shape.txt"
                         print(" outfile_shape ", outfile_shape)
                         command_shape = f"combineCards.py {outName} -S > {outfile_shape}"
                         print(command_shape)
                         # Run the command with shell=True to enable redirection
                         subprocess.call(command_shape, shell=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is_int", dest = "is_int",action="store_true", default = False, help="consider interference for CI")
    ...
    print(f"args.expected: {args.expected}")
    print(f"args.redo: {args.redo}")
    print(f"Debugging: args.is_int = {args.is_int}")
    print(f"args.write: {args.write}")
    print(f"args.submit: {args.submit}")
    ...
    if args.crab:
        submitLimitsToCrab(args,config,cardDir, tag)

    if (args.redo or args.write) and not args.LEE and not (args.BBLL or args.BSLL):
        createInputs(args,config,cardDir)
    elif (args.redo or args.write) and not args.LEE and (args.BBLL or args.BSLL):
        print(f"args.write: {args.write}")
        print(f"args.redo: {args.redo}")
        
        print(f"args.is_int: {args.is_int}")
        print(f"args.expected: {args.expected}")

        
        if (args.is_int and args.expected):
            print("runInterpretation script flag 1")
            createInputsGen(args,config,cardDir+"_expGen")
            #subprocess_args = ["python3","runInterpretation_default.py","-w","-c","%s"%args.config,"-t","%s_expGen"%tag,"--BBLL"]
            #subprocess.call(subprocess_args)
            #if args.workDir == "":
            #    print("this is working 0.3")
            #    subprocess.call(["python3","runInterpretation.py","-w","-c","%s"%args.config,"-t","%s_expGen"%tag,"--BBLL"])
            #else:
            #    subprocess.call(["python3","runInterpretation.py","-w","-c","%s"%args.config,"-t","%s_expGen"%tag,"--BBLL","--workDir",args.workDir])
            prepareToys(args,config,cardDir+"_expGen")
        if args.BBLL: 
            print("args BBLL!")
            createInputsBBLL(args,config,cardDir)
            ...
            if args.signif:
                if not args.LEE:
                    ...
                    for i in range(0,1000):
                        ...
                        cardDir = getCardDir(args,config)
                        if args.redo:
                            createInputs(args,config,cardDir,toysOnly=True)
                        submitPValues(args,config,outDir,args.binned,args.tag)						
                        i += 1	
                        args.tag=tagSave					
            else:
                print("submitting limits on slurm")
