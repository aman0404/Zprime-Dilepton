...
labels = {"Run2": "combination","Dielectron": "dielectron channel","Dimuon": "dimuon channel"}
lineStyles = {"Run2": 1,"Dielectron": 2,"Dimuon": 3}
lineColors = {"Run2":ROOT.kBlack,"Dielectron":  ROOT.kRed,"Dimuon": ROOT.kBlue}


labelsWidth = {"signif": "#Gamma_{Z'}/M_{Z'} = 0.6%","signif006": "#Gamma_{Z'}/M_{Z'} = 0.6%","signif01": "#Gamma_{Z'}/M_{Z'} = 1%","signif03": "#Gamma_{Z'}/M_{Z'} = 3%","signif05": "#Gamma_{Z'}/M_{Z'} = 5%","signif10": "#Gamma_{Z'}/M_{Z'} = 10%"}
...
def main():
		...
		graphs = []
		if 'width' in args.config:
			width = "signif"+card.split('width')[-1].split('_')[0]
		else:
			width = 'signif'
	
		name = "pValuesChannel_%s_%s"%(args.config,args.tag)
		leg = ROOT.TLegend(0.32, 0.72, 0.9, 0.875,"%s"%labelsWidth[width],"brNDC")
		leg.SetFillColor(10)
		leg.SetLineColor(10)
		...
		for index, card in enumerate(args.cards):
			...
			graphs[index].SetLineColor(lineColors[channel])
			graphs[index].SetLineStyle(lineStyles[channel])
			leg.AddEntry(graphs[index],labels[channel],"l")			
		...
		plotPad.SetLogx()
		canv.Print("plots/"+name+"_log.pdf")
		canv.Print("plots/"+name+"_log.root")
