...
TFile *f1 = TFile::Open("out_response_matrix_mu_bb_script_2b.root");
...
TFile *f3 = TFile::Open("../../../gen/mumu/2b_bb_2018.root");
...
std::cout<<"hmu_data "<<hmu_data->GetBinContent(1)<<'\t'<<hmu_data->GetBinContent(2)<<std::endl;
...
TFile *file1 = new TFile("unfolded_mc_muon_bb_data_2b.root","RECREATE");
file1->cd();
o->Write();
oMC->Write();
hmu_data_org->Write();
