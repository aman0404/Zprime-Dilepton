...
TFile *f1 = TFile::Open("out_response_matrix_mu_bb_script_0b.root");
...
TFile *f2 = TFile::Open("../../../reco/mumu/0b_bb_2018.root");
f2->GetObject("mu_data_obs", hmu_data_unbin);

TH1F *hmu_data = (TH1F*)hmu_data_unbin->Rebin(7,"hmu_data",bins);

TFile *f3 = TFile::Open("../../../gen/mumu/0b_bb_2018.root");
f3->GetObject("DYJets", hmu_gen_unbin);

TH1F *hmu_gen = (TH1F*)hmu_gen_unbin->Rebin(7,"hmu_gen",bins);

TFile *f4 = TFile::Open("../../../reco/mumu/0b_bb_2018.root");
f4->GetObject("DYJets", hmu_reco_unbin);

TH1F *hmu_reco = (TH1F*)hmu_reco_unbin->Rebin(7,"hmu_reco",bins);

hmu_reco->Scale(0.95);

TH1 *htop, *hdiboson, *hfake;

TFile *f5fake = TFile::Open("../../../reco/mumu/fake_0b_bb_2018.root");
...
TFile *f5 = TFile::Open("../../../reco/mumu/0b_bb_2018.root");
...
TH1F *hbkg = (TH1F*)htop->Rebin(7,"hbkg",bins);
...
TFile *file1 = new TFile("unfolded_mc_muon_bb_data.root","RECREATE");
file1->cd();
o->Write();
oMC->Write();
hmu_data_org->Write();
