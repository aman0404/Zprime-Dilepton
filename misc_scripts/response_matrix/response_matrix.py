...
df = df[(df["r"]==f"{parameters['regions']}") & (df["dimuon_mass"] > 200.) & (df["dimuon_mass_gen"] > 200) & (df["nbjets"]==0)]


reco_mass = df["dimuon_mass"].compute().values

gen_mass = df["dimuon_mass_gen"].compute().values

wgt = df["wgt_nominal"].compute().values

wgt_ID_up = df["wgt_muID_up"].compute().values
wgt_ID_dn = df["wgt_muID_down"].compute().values 

wgt_l1prefire_up = df["wgt_l1prefiring_wgt_up"].compute().values
wgt_l1prefire_dn = df["wgt_l1prefiring_wgt_down"].compute().values

wgt_res = df["dimuon_mass_resUnc"].compute().values

wgt_reco = df["wgt_recowgt_down"].compute().values

wgt_pu_up = df["wgt_pu_wgt_up"].compute().values
wgt_pu_dn = df["wgt_pu_wgt_down"].compute().values

wgt_scale_up = df["dimuon_mass_scaleUncUp"].compute().values
wgt_scale_dn = df["dimuon_mass_scaleUncDown"].compute().values


wgt_iso_up = df["wgt_muISO_up"].compute().values
wgt_iso_dn = df["wgt_muISO_down"].compute().values

wgt_hlt_up = df["wgt_muHLT_up"].compute().values
wgt_hlt_dn = df["wgt_muHLT_down"].compute().values
...
hist2dmu = TH2F("response_mu","response_mu", 7 ,array('d',binsx), 7, array('d',binsx))

hist2dmu_id_up    = TH2F("hist2dmu_id_up","hist2dmu_id_up", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_id_dn    = TH2F("hist2dmu_id_dn","hist2dmu_id_dn", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_l1_up    = TH2F("hist2dmu_l1_up","hist2dmu_l1_up", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_l1_dn    = TH2F("hist2dmu_l1_dn","hist2dmu_l1_dn", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_pu_up    = TH2F("hist2dmu_pu_up","hist2dmu_pu_up", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_pu_dn    = TH2F("hist2dmu_pu_dn","hist2dmu_pu_dn", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_iso_up   = TH2F("hist2dmu_iso_up","hist2dmu_iso_up", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_iso_dn   = TH2F("hist2dmu_iso_dn","hist2dmu_iso_dn", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_hlt_up   = TH2F("hist2dmu_hlt_up","hist2dmu_hlt_up", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_hlt_dn   = TH2F("hist2dmu_hlt_dn","hist2dmu_hlt_dn", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_reco     = TH2F("hist2dmu_reco","hist2dmu_reco", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_pdf_up   = TH2F("hist2dmu_pdf_up","hist2dmu_pdf_up", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_pdf_dn   = TH2F("hist2dmu_pdf_dn","hist2dmu_pdf_dn", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_res      = TH2F("hist2dmu_res","hist2dmu_res", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_scale_up = TH2F("hist2dmu_scale_up","hist2dmu_scale_up", 7 ,array('d',binsx), 7, array('d',binsx))
hist2dmu_scale_dn = TH2F("hist2dmu_scale_dn","hist2dmu_scale_dn", 7 ,array('d',binsx), 7, array('d',binsx))
...
file1 = TFile("out_response_matrix_mu_be_script_0b.root","RECREATE")
file1.cd()
hist2dmu.Write()
